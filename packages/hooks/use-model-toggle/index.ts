...
import { buildProp, definePropType, isBoolean, isFunction } from '@hyper/utils';
...
export const createModelToggleComposable = <T extends string>(name: T) => {
    ...
    const useModelToggle = ({
        ...
        const props = instance.props as _UseModelToggleProps<T> & { disabled: boolean };
        const { emit } = instance;
        ...
        const isModelBindngAbsent = computed(() => props[name] === null);
        ...
        const show = () => {
            ...
            if(isModelBindngAbsent.value || !shouldEmit) doShow();
        };
        ...
        const hide = () => {
            ...
            if(isModelBindngAbsent.value || !shouldEmit) doHide();
        };
        ...
        onMounted(() => {
            onChange(props[name] as unknown as boolean);
        });
